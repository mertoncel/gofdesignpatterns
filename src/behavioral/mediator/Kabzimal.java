...
public class Kabzimal implements Araci {
    ...
    public void ureticidenAl(Uretici uretici) {
        ...
        Halci halci = enUygunHalciyiBul(uretici.getUrun());
    ...
    public void haleSat(Halci halci) {
        ...
        Uretici uretici = enUygunUreticiyiBul(halci.getUrun());
        ...
        for (Uretici uretici : ureticiList) {
            if (urun.equals(uretici.getUrun())) {
                ...
                if (enUygunUretici == null ||
                        enUygunUretici.getFiyat().compareTo(ureticiFiyat) > 0) {
    ...
    private Halci enUygunHalciyiBul(EnumUrun urun) {
        ...
        for (Halci halci : halciList) {
            if (urun.equals(halci.getUrun())) {
                BigDecimal halciFiyat = halci.getFiyat();

                if (enUygunHalci == null ||
                        enUygunHalci.getFiyat().compareTo(halciFiyat) < 0) {
                    enUygunHalci = halci;
                }
            }
        }
    ...
    private void satisYap(Uretici uretici, Halci halci) {
        System.out.println(uretici.getAdi() + " isimli uretici "
                + halci.getAdi() + " isimli halciye " + uretici.getUrun().getUrun() + " sattu");
    }
}
